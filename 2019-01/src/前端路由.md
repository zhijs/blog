...
1. 根据传入的 routers 创建一个路由映射器，也就是 createMatcher 所做的事。createMatcher 利用了闭包，内部存储的了路径列表，名称到路由的映射表，路径到路由的映射表，这个路由映射器的定义如下所示
```javascript
export type Matcher = {
...
那我们继续来看下，初始化 history 属性的过程，这里我们只看 HashHistory，首先 HashHistory 继承自 History， HashHistory 的定义如下所示：  
```javascript
...
```
可以看到 HashHistory 中，主要使为当前的 window 对象添加了一个浏览器历史记录更改事件，如果支持 popstate 的话就用 popstate 事件，否则使用 hashchange，然后在记录改变的情况下调用 transitionTo 方法，这个方法定义在其父类 history 中。其定义如下所示:  
```javascript
export class History {
  ...
  transitionTo (location: RawLocation, onComplete?: Function, onAbort?: Function) {
    // 
    const route = this.router.match(location, this.current) // 匹配将要导航到的路由
